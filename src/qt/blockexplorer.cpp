...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::DIN, nValue);
...
{
    std::string genesisblockhash = "000001831fa6b4c3e1590c6aee28dba04d7a58b72f6431c3e6ebf656cd28e952";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (dinero.conf).");
            QMessageBox::warning(this, "DINERO Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
