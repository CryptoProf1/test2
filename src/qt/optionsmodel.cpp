...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DIN);
    ...
    if (!settings.contains("nAnonymizeDineroAmount"))
        settings.setValue("nAnonymizeDineroAmount", 1000);

    nAnonymizeDineroAmount = settings.value("nAnonymizeDineroAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDineroAmount"))
        SoftSetArg("-anonymizedineroamount", settings.value("nAnonymizeDineroAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dinero.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDineroAmount:
            return QVariant(nAnonymizeDineroAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDineroAmount:
            nAnonymizeDineroAmount = value.toInt();
            settings.setValue("nAnonymizeDineroAmount", nAnonymizeDineroAmount);
            emit anonymizeDineroAmountChanged(nAnonymizeDineroAmount);
            break;
        case CoinControlFeatures:
