...
#include "ui_zdincontroldialog.h"
...
std::list<std::string> ZDinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDinControlDialog::listMints;

ZDinControlDialog::ZDinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDinControlDialog::~ZDinControlDialog()
{
    delete ui;
}

void ZDinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDinControlDialog::updateList()
{
...
void ZDinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDinControlDialog::GetSelectedMints()
{
...
void ZDinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
